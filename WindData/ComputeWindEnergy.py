...
WindTurbine="ERGIS" #Name of the wind turbine file
...
def Condition_on_TurbineData(WindTurbine, Range_of_WindSpeeds):
    ...
    return AdjustedHubHeight, RotorDiameter, PowerCurve, RatedPower


#Convert wind speed to energy in pu
def WindToEnergy(AdjustedHubHeight, PowerCurve, WindTurbine):
    ...
    WindEnergy=Modified_PowerCurve[IntValuesWindSpeed]
    WindEnergy=WindEnergy.astype('float32')
    

    ReadMe='\
    EnergyPu: pu wind energy (Base 2MW)-1 unit per site\n\
...
WindToEnergy(AdjustedHubHeight, PowerCurve, WindTurbine)
